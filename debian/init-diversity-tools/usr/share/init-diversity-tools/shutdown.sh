		...
		if [ $(cat /proc/cmdline | grep -ic "init=/") -eq 0 ]; then
			echo "Primary Live/Frugal init booted... detecting primary live init"
				if [ $(ps -p1 | grep -ic "runit") -eq 1 ]; then
					echo "runit Live/Frugal system detected...  Shutdown..."
					/usr/lib/runit/shutdown $@;
					exit 0
				elif [ $(ps -p1 | grep -ic "s6-svscan") -eq 1 ] && [ -e /run/s6-rc ]; then
					echo "s6-rc Live/Frugal system detected...  Shutdown..."
					openvt -c12 -f -- /usr/bin/s6-linux-init-shutdown $@;
					exit 0
				elif [ $(ps -p1 | grep -ic "s6-svscan") -eq 1 ] && [ -e /run/66 ]; then
					echo "s6-66 Live/Frugal system detected...  Shutdown..."
					/usr/bin/66-shutdown $@;
					exit 0
				elif [ $(ps -p1 | grep -ic "openrc") -eq 1 ] || [ $(readlink -e /sbin/init | grep -ic "openrc") -eq 1 ]; then
					echo "openrc Live/Frugal system detected...  Shutdown..."
					/usr/lib/openrc/openrc-shutdown $@ | /usr/sbin/openrc-shutdown $@;
					exit 0
				elif [ $(ps -p1 | grep -ic "dinit") -eq 1 ] || [ $(/sbin/init --version | grep -ic "Dinit") -eq 1 ]; then
					echo "dinit Live/Frugal system detected...  Shutdown..."
					/usr/lib/dinit/shutdown $@
					exit 0
				elif [ $(ps -p1 | grep -ic "init") -eq 1 ] && [ $(/sbin/init --version | grep -ic "SysV") -eq 1 ]; then
					echo "sysvinit Live/Frugal system detected...  Shutdown..."
					/usr/lib/sysvinit/shutdown $@
					exit 0
				elif [ $(ps -p1 | grep -ic "systemd") -eq 1 ]; then
					echo "systemd Live/Frugal system detected...  Shutdown..."
					systemctl shutdown $@ | /usr/lib/systemd/systemd-shutdown $@;
					exit 0
				else 
					echo "Strange... Cannot detect Live/Frugal init...  Will try to reboot using bysybox... Shutdown..."
	...
	echo "Installed system detected... detecting init"
		if [ $(cat /proc/cmdline | grep -ic "init=/") -eq 0 ]; then
			echo "Primary init booted... detecting primary init"
				if [ $(ps -p1 | grep -ic "runit") -eq 1 ]; then
					echo "runit primary init detected...  Shutdown..."
					/usr/lib/runit/shutdown $@;
					exit 0
				elif [ $(ps -p1 | grep -ic "s6-svscan") -eq 1 ] && [ -e /run/s6-rc ]; then
					echo "s6-rc primary init detected...  Shutdown..."
					/usr/bin/s6-linux-init-shutdown $@;
					exit 0
				elif [ $(ps -p1 | grep -ic "s6-svscan") -eq 1 ] && [ -e /run/66 ]; then
					echo "s6-66 primary init detected...  Shutdown..."
					/usr/bin/66-shutdown $@;
					exit 0
				elif [ $(ps -p1 | grep -ic "openrc") -eq 1 ] || [ $(readlink -e /sbin/init | grep -ic "openrc") -eq 1 ]; then
					echo "openrc primary init detected...  Shutdown..."
					/usr/lib/openrc/openrc-shutdown $@ | /usr/sbin/openrc-shutdown $@;
					exit 0
				elif [ $(ps -p1 | grep -ic "dinit") -eq 1 ] || [ $(/sbin/init --version | grep -ic "Dinit") -eq 1 ]; then
					echo "dinit primary init detected...  Shutdown..."
					/usr/lib/dinit/shutdown $@
					exit 0
				elif [ $(ps -p1 | grep -ic "init") -eq 1 ] && [ $(/sbin/init --version | grep -ic "SysV") -eq 1 ]; then
					echo "sysvinit primary init detected...  Shutdown..."
					/usr/lib/sysvinit/shutdown $@
					exit 0
				elif [ $(ps -p1 | grep -ic "systemd") -eq 1 ]; then
					echo "systemd primary init detected...  Shutdown..."
					systemctl shutdown $@ | /usr/lib/systemd/systemd-shutdown $@;
					exit 0
				else 
					echo "Strange... Cannot detect primary init...  Will try to reboot using bysybox... Shutdown..."
					sync && sleep 5 && /usr/bin/busybox shutdown -f;
					exit 1
				fi
		else
			echo "cmdline init booted... detecting init"
				if [ $(ps -p1 | grep -ic "runit") -eq 1 ]; then
					echo "runit init detected...  Shutdown..."
					/usr/lib/runit/shutdown $@;
					exit 0
				elif [ $(ps -p1 | grep -ic "s6-svscan") -eq 1 ] && [ -e /run/s6-rc ]; then
					echo "s6-rc init detected...  Shutdown..."
					/usr/bin/s6-linux-init-shutdown $@;
					exit 0
				elif [ $(ps -p1 | grep -ic "s6-svscan") -eq 1 ] && [ -e /run/66 ]; then
					echo "s6-66 init detected...  Shutdown..."
					/usr/bin/66-shutdown $@;
					exit 0
				elif [ $(ps -p1 | grep -ic "openrc") -eq 1 ]; then
					echo "openrc init detected...  Shutdown..."
					/usr/lib/openrc/openrc-shutdown $@ | /usr/sbin/openrc-shutdown $@;
					exit 0
				elif [ $(ps -p1 | grep -ic "dinit") -eq 1 ] || [ $(cat /proc/cmdline | grep -ic "dinit") -eq 1 ]; then
					echo "dinit init detected...  Shutdown..."
					/usr/lib/dinit/shutdown $@
					exit 0
				elif [ $(ps -p1 | grep -ic "init") -eq 1 ] && [ $(cat /proc/cmdline | grep -ic "sysvinit") -eq 1 ]; then
					echo "sysvinit init detected...  Shutdown..."
					/usr/lib/sysvinit/shutdown $@
					exit 0
				elif [ $(ps -p1 | grep -ic "systemd") -eq 1 ]; then
					echo "systemd init detected...  Shutdown..."
					systemctl shutdown $@ | /usr/lib/systemd/systemd-shutdown $@;
					exit 0
				else 
					echo "Strange... Cannot detect cmdline init...  Will try to reboot using bysybox... Shutdown..."
					sync && sleep 5 && /usr/bin/busybox shutdown -f;
					exit 1
				fi	
		fi
